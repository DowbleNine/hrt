...
app.post('/api/runes/save', async (req, res) => {
  const { user_email, spell_name, core_id, vector_id, modulator_ids, power } = req.body;

  if (!user_email || !spell_name || !core_id) {
    return res.status(400).json({ status: 'error', message: 'Campos obrigatórios ausentes.' });
  }

  try {
    const { error } = await supabase.from('vga_runes').insert({
      user_email,
      spell_name,
      core_id,
      vector_id,
