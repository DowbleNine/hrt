...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Mantemos uma verificação simples de login
    const user = localStorage.getItem('userTier'); // Ou qualquer outra chave de sessão
    if (!user) {
      navigate('/login');
    }
  }, [navigate]);
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h2 className="text-xl font-bold text-emerald-400 border-b border-gray-800 pb-2">Treinamento Principal</h2>
              ...
              {[1, 2, 3, 4].map((i) => (
                <motion.div whileHover={{ y: -5 }} key={i} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden group cursor-pointer transition-colors hover:border-emerald-500/50">
                  ...
                  <div className="p-4">
                    ...
                    <h3 className="font-bold text-gray-200">Aula {i}</h3>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-lime-400 border-b border-gray-800 pb-2">Suporte & Comunidade</h2>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <h3 className="text-lg font-black mb-2 text-white">Mentoria & FAQ</h3>
              <p className="text-sm text-gray-400 mb-6">Acesse as gravações das calls e o suporte.</p>
              
              <button className="w-full bg-lime-500 hover:bg-lime-400 text-black font-black uppercase py-3 rounded transition-colors">
                Ver Gravações
              </button>
            </div>
          </div>
